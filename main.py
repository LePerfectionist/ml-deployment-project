...
@app.get("/")
def root():
    return {"message": "Mushroom classifier is alive 🍄"}
...
def predict(data: MushroomInput):
    ...
    input_encoded = [
        encoders[col].transform([input_data[col]])[0] for col in input_data
    ]
    prediction = model.predict([input_encoded])[0]
    edible = encoders['class'].inverse_transform([prediction])[0]
    return {"prediction": "Edible" if edible == 'e' else "Poisonous"}
